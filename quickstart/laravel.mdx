...
::: tip Queue Types
While we recommend using a unicast queue with a single worker for most
Laravel applications,
you may also configure your queue as a multicast queue. This allows
jobs to be handled by multiple workers at once (which can be at a
completely different server) allowing you more flexibility in
...
:::tip Using Shove in Development Environments
You must have a public URL for Shove to send jobs to. If you are running
your Laravel application locally, you can use a service
like [ngrok](https://ngrok.com/) to create a public URL for your local
server.
:::
...
::: tip Custom Shove Endpoints
Feel free to use a custom endpoint for your incoming Shove jobs. Just
...
Since Shove doesn’t use application sessions, you need to disable this
...
:::tip Laravel 11
If you’re using Laravel 11, Shove handles disabling CSRF verification
automatically.
:::
...
class PodcastStoreController extends Controller
{
    public function __invoke(Request $request)
    {
        ...
        dispatch(new ProcessPodcastEpisode($episode)) // [!code focus]
            ->onConnection('shove') // [!code focus]
            ->onQueue('podcasts'); // [!code focus]
